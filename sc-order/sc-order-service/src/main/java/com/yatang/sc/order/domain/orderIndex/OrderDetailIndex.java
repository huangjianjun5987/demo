...
public class OrderDetailIndex implements Serializable {

    private static final long serialVersionUID = -5718681241947511698L;
    ...
    private List<OrderItemIndex> orderItemPoList;//订单商品详情
    private List<PaymentGroupIndex> paymentGroupList;//付款记录
    private List<RefundIndex> refundList;//退款记录
    ...
    public List<OrderItemIndex> getOrderItemPoList() {
        return orderItemPoList;
    }

    public List<PaymentGroupIndex> getPaymentGroupList() {
        return paymentGroupList;
    }

    public void setOrderItemPoList(List<OrderItemIndex> orderItemPoList) {
        if (CollectionUtils.isEmpty(orderItemPoList)) {
            this.orderItemPoList = new ArrayList<OrderItemIndex>();
        } else {
            this.orderItemPoList = orderItemPoList;
        }
    }

    public void setPaymentGroupList(List<PaymentGroupIndex> paymentGroupList) {
        if (CollectionUtils.isEmpty(paymentGroupList)) {
            this.paymentGroupList = new ArrayList<PaymentGroupIndex>();
        } else {
            this.paymentGroupList = paymentGroupList;
        }
    }

    public List<RefundIndex> getRefundList() {
        return refundList;
    }

    public void setRefundList(List<RefundIndex> refundList) {
        if (CollectionUtils.isEmpty(refundList)) {
            this.refundList = new ArrayList<RefundIndex>();
        } else {

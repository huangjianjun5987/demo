...
import com.yatang.sc.xinyi.service.kidd.XinyiOrderCancelProxyService;
...
/**
 * @描述:心怡订单服务(取消>>采购和销售,订单通知>>销售和采购)
 * @类名:XinyiOrderCancelServiceImpl
 * @作者: lvheping
...
public class XinyiOrderServiceImpl {
    ...
    private final XinyiOrderCancelProxyService xinyiOrderCancelProxyService;
    ...
    public Response<String> cancel(KiddCancelAllOrdersDto kiddCancelAllOrdersDto) {
        ...
        KiddUtils.checkXinyiListening( kiddFacadeService.warehouseInterfaceProvider( kiddCancelAllOrdersDto.getOrders().get( 0 ).getWarehouseCode() ) );
        ...
        try {
            ...
            if (KiddOrderLogisticsType.KIDD_CGRK.equals(orderType)) {//取消采购单
                for (KiddCancelOrdersDto kiddCancelOrdersDto : orders) {
                    ...
                    ResponseDto responseDto = xinyiOrderCancelProxyService.cancel( orderCancelRequestDto );
                    log.info("service----cancel>>心怡处理采购订单取消--返回:{}", JSON.toJSONString( responseDto ) );
                    if (Objects.equal( responseDto.getFlag(), ResponseDto.SUCCESS )) {//请求成功
                        ...
                        log.error( "service----cancel>>心怡处理采购订单取消失败响应码:{},失败原因:{}", responseDto.getCode(), responseDto.getMessage() );
                ...
                if (Objects.equal(responseDto.getFlag(), ResponseDto.SUCCESS)) {//请求成功
                    ...
                    kiddConfirmEntryOrderDto.setStatus(KiddOrderLogisticsType.KIDD_CANCELED);
                    kiddConfirmEntryOrderDto.setEntryOrderCode(kiddCancelAllOrdersDto.getOrders().get(0).getOrderCode());
                    kiddPurchaseOrderConfirmDto.setEntryOrder(kiddConfirmEntryOrderDto);
                    log.info("service----cancel>>心怡处理采购退货单取消成功,发送mq:{}", JSONObject.toJSONString(kiddPurchaseOrderConfirmDto));
                    ...
                    KiddPurchaseOrderConfirmDto kiddPurchaseOrderConfirmDto=new KiddPurchaseOrderConfirmDto();
                    KiddConfirmEntryOrderDto kiddConfirmEntryOrderDto=new KiddConfirmEntryOrderDto();
                    kiddConfirmEntryOrderDto.setStatus(KiddOrderLogisticsType.KIDD_CANCELEDFAIL);
                    kiddConfirmEntryOrderDto.setEntryOrderCode(kiddCancelAllOrdersDto.getOrders().get(0).getOrderCode());
                    kiddPurchaseOrderConfirmDto.setEntryOrder(kiddConfirmEntryOrderDto);
                    log.info("service----cancel>>心怡处理采购退货单取消失败,发送mq:{}", JSONObject.toJSONString(kiddPurchaseOrderConfirmDto));
                    purchaseRefundMQProducer.sendMsg(kiddPurchaseOrderConfirmDto);
                ...
                //请求心怡取消动作
                ResponseDto responseDto = xinyiOrderCancelProxyService.cancel( orderCancelRequestDto );
                log.info( "service----cancel>>心怡处理销售订单取消--返回:{}", JSON.toJSONString( responseDto ) );
                //设置mq发送
                ...
                if (Objects.equal( responseDto.getFlag(), ResponseDto.SUCCESS )) {//请求成功
                    ...
                    log.error( "service----cancel>>心怡处理销售订单取消失败响应码:{},失败原因:{}", responseDto.getCode(), responseDto.getMessage() );
                    response.setSuccess( false );
	...
	private void dataConvert(OrderCancelRequestDto orderCancelRequestDto, KiddCancelOrdersDto kiddCancelOrdersDto) {
     ...
     */
    @EventListener
    public Response<Void> getOrderNotice(XinyiOrderNoticeRequestInfoDto noticeRequestInfoDto) {
        log.info( "service--getOrderNotice>>心怡订单通知分发信息:{}", JSON.toJSONString( noticeRequestInfoDto ) );
        ...
        try {
            ...
            if (KiddOrderLogisticsType.KIDD_CGRK.equals(orderType)) {//发送采购mq
                log.info( "service--getOrderNotice>>心怡订单通知分发信息编号:{},发送信息:{},走采购订单mq发送", noticeRequestInfoDto.getOrder().getOrderCode(), JSON.toJSONString( requestDto ) );
                //  entryOrderMQProducer.sendOrderlyMsg(noticeRequestInfoDto.getOrder().getOrderCode(), JSON.toJSONString(requestDto));
                XinyiOrderNoticeInfoDto order = noticeRequestInfoDto.getOrder();
                XinyiOrderProcessInfoDto process = noticeRequestInfoDto.getProcess();
                KiddPurchaseOrderConfirmDto kiddPurchaseOrderConfirmDto = new KiddPurchaseOrderConfirmDto();
                KiddConfirmEntryOrderDto kiddConfirmEntryOrderDto = new KiddConfirmEntryOrderDto();//采购单基本信息
                ...
                log.info( "service--getOrderNotice>>心怡订单通知分发信息编号:{},发送信息:{},走销售订单mq发送", noticeRequestInfoDto.getOrder().getOrderCode(), JSON.toJSONString( requestDto ) );

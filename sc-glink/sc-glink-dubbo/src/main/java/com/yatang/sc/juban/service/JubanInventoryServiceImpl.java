...
import com.yatang.sc.juban.dto.im.JubanInventoryAdjustNoticeItemDto;
...
import com.yatang.sc.juban.dto.im.JubanInventoryQueryItemDto;
...
@Slf4j
@Service
public class JubanInventoryServiceImpl {
    ...
    public Response<ImAdjustmentResultDto> inventoryQuery(InventoryQueryDto inventoryQueryDto) {
        log.info("查询桔瓣实时库存消息 inventoryQueryDto{}" , JSON.toJSONString(inventoryQueryDto));
        ...
        KiddUtils.checkJubanListening(kiddFacadeService.warehouseInterfaceProvider(inventoryQueryDto.getWarehouseCode()));
        ...
        try {
            ...
            if (inventoryQueryDto.getItemCode() == null) {
                ...
                if (strings != null&&strings.size()>0) {
                        List<JubanInventoryQueryCriteriaDto> list = new ArrayList<>();
                        JubanInventoryQueryRequestDto inventoryQueryRequestDto = new JubanInventoryQueryRequestDto();
                        for (String proCode : strings) {
                            JubanInventoryQueryCriteriaDto inventoryQueryCriteriaDto = new JubanInventoryQueryCriteriaDto();
                            ...
                            if (list.size() % 500 == 0) {//每五百个商品调用一次
                                ...
                                log.info("查询桔瓣库存具体参数信息list {}" , JSON.toJSONString(list));
                                JubanInventoryQueryResponseDto inventoryQuery = jubanProductProxyService.inventoryQuery(inventoryQueryRequestDto);
                                if (inventoryQuery != null) {
                                    List<JubanInventoryQueryItemDto> items = inventoryQuery.getItems();
                                    if (items != null && items.size() > 0) {
                                        for (JubanInventoryQueryItemDto itemDto : items) {
                        ...
                        //最后一次商品调用
                        inventoryQueryRequestDto.setCriteriaList(list);
                        log.info("最后一次商品调用查询桔瓣库存具体参数信息list {}" , JSON.toJSONString(list));
                        JubanInventoryQueryResponseDto inventoryQuery = jubanProductProxyService.inventoryQuery(inventoryQueryRequestDto);
                        log.info("最后一次商品调用查询桔瓣库存信息inventoryQuery {}" , JSON.toJSONString(inventoryQuery));
                        if (inventoryQuery != null) {
                            List<JubanInventoryQueryItemDto> items = inventoryQuery.getItems();
                            if (items != null && items.size() > 0) {
                                for (JubanInventoryQueryItemDto itemDto : items) {
                ...
                JubanInventoryQueryRequestDto requestDto = new JubanInventoryQueryRequestDto();
                JubanInventoryQueryCriteriaDto criteriaDto = BeanConvertUtils.convert(inventoryQueryDto, JubanInventoryQueryCriteriaDto.class);
                criteriaDto.setItemId(inventoryQueryDto.getItemCode());
                requestDto.setCriteriaList(Lists.newArrayList(criteriaDto));
                log.info("查询桔瓣仓库库存信息所用参数requestDto {}" , JSON.toJSONString(requestDto));
                JubanInventoryQueryResponseDto inventoryQuery = jubanProductProxyService.inventoryQuery(requestDto);
                log.info("查询桔瓣仓库库存信息结果inventoryQuery {}" , JSON.toJSONString(inventoryQuery));
                if (inventoryQuery != null) {
                    List<JubanInventoryQueryItemDto> items = inventoryQuery.getItems();
                    if (items != null && items.size() > 0) {
                        for (JubanInventoryQueryItemDto itemDto : items) {
     ...
     */
    @EventListener
    public Response<Void> adjustInventoryReceiptNotice(JubanInventoryAdjustNoticeRequestDto adjustNoticeRequestDto) {

        log.info("service---adjustInventoryReceiptNotice>>桔瓣发送库存调整单通知", JSON.toJSONString(adjustNoticeRequestDto));
        ...
        try {
            ...
            for (JubanInventoryAdjustNoticeItemDto adjustNoticeItemDto : adjustNoticeRequestDto.getItems()) {
            ...
            log.error("桔瓣库存调整单接收失败+" + ExceptionUtils.getFullStackTrace(e));

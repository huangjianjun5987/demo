...
public class ApplicationListenerResultMethodAdapter implements GenericApplicationListener {
    ...
    public ApplicationListenerResultMethodAdapter(String beanName, Class<?> targetClass, Method method) {
    ...
    public boolean supportsEventType(ResolvableType eventType) {
        for (ResolvableType declaredEventType : this.declaredEventTypes) {
            if (declaredEventType.isAssignableFrom(eventType)) {
                return true;
            } else if (PayloadApplicationResultEvent.class.isAssignableFrom(eventType.getRawClass())) {
                ResolvableType payloadType = eventType.as(PayloadApplicationResultEvent.class).getGeneric();
     ...
     */
    public Object processEvent(ApplicationEvent event) {
    ...
    protected Object[] resolveArguments(ApplicationEvent event) {
        ...
        if (!ApplicationEvent.class.isAssignableFrom(declaredEventType.getRawClass()) &&
                event instanceof PayloadApplicationResultEvent) {
            return new Object[]{((PayloadApplicationResultEvent) event).getPayload()};
        } else {
    ...
    private ResolvableType getResolvableType(ApplicationEvent event) {
        ...
        if (event instanceof PayloadApplicationResultEvent) {
            PayloadApplicationResultEvent<?> payloadEvent = (PayloadApplicationResultEvent<?>) event;
            payloadType = payloadEvent.getResolvableType().as(PayloadApplicationResultEvent.class).getGeneric();
        }
        for (ResolvableType declaredEventType : this.declaredEventTypes) {

...
public class ApiResponseBodyAdvice implements ResponseBodyAdvice {
  ...
  public Object beforeBodyWrite(Object body, MethodParameter returnType, MediaType selectedContentType, Class selectedConverterType, ServerHttpRequest request, ServerHttpResponse response) {
    ...
    builder.isSuccess(bodyResponse.isSuccess());
    try {
      builder.code(Integer.parseInt(bodyResponse.getCode()));
    }catch (Exception e){
      builder.code(500);
    }
    builder.message(bodyResponse.getErrorMessage());
    boolean isNullArray = isNullArray(returnType, bodyResponse);
    builder.data(isNullArray ? Collections.emptyList() : bodyResponse.getResultObject());
    return builder.build();
  }

  private boolean isNullArray(MethodParameter returnType, Response bodyResponse) {
    Type returnGenericType = returnType.getGenericParameterType();
    if (!(returnGenericType instanceof ParameterizedType)) {
      return false;
    }

    Type resultObjectType = ((ParameterizedType) returnType.getGenericParameterType()).getActualTypeArguments()[0];
    ...
    if (resultObjectType instanceof Class) {
      ...
      resultObjectClass = (Class) ((ParameterizedType) resultObjectType).getRawType();
    }
    ...
    return (Lists.newArrayList(List.class, Set.class).contains(resultObjectClass) || resultObjectClass.isArray()) && bodyResponse.getResultObject() == null;
